...
class MultiScaleJD(LoadImagesAndLabels):
    ...
    def __init__(self,
        ...
        #./mydata/visdrone.train
        self.paths = paths
        # self.path = {"visdrone": "./mydata/visdrone.train"}
        # ds = visdrone path = './mydata/visdrone.train'
        ...
        # NOTE:images address storage in img_files[visdrone]
        # labels address storage in label_files[visdrone]
                ...
                for lp in label_paths:
                    ...
                    with warnings.catch_warnings():
                        ...
                        for item in lb:  # label中每一个item(检测目标)
                            # if item id > max_ids_dict[cls_id], modify max_ids_dict[cls_id] = item id
    ...
    def __getitem__(self, idx):
        ...
        #生成前一帧的图像
        # begin = 0
        # if int(img_path.split('/')[-1].split(".jpg")[0]) == 1:
        #     begin = 1
        ...
        pre_img_path = os.path.join('/mnt/sdd/public/data/wp/MOT/Dataset/VisDrone_Track/gendataset',video_path[0],video_path[1],video_path[2],pre_image_id+'.jpg')
        
        # pre_label_path = os.path.join('/media/ls/FA5ECF148688E396/DataSet/visdrone_fmot', ann_path[0],ann_path[1],ann_path[2], pre_image_id+'.txt')
        pre_label_path = os.path.join('/mnt/sdd/public/data/wp/MOT/Dataset/VisDrone_Track/gendataset', ann_path[0],ann_path[1],ann_path[2], pre_image_id+'.txt')
        if not os.path.isfile(pre_img_path):
            ...
            pre_imgs, pre_labels, pre_img_path, (pre_input_h, pre_input_w) = self.get_data(pre_img_path, pre_label_path, width, height)
        ...
        if self.opt.id_weight > 0:
            ret = {'input': imgs,
                   ...
                   'pre_location_class': pre_location_class,
                #    'begin': begin,
                   }
...
class MYMultiScaleJD(LoadImagesAndLabels):
    """
    multi-joint scale for trainig
    """
    mean = None
    ...
    def __getitem__(self, idx):
        ...
        video_path = img_path.split('/')[-4:-1]
        ann_path = label_path.split('/')[-4:-1]
        pre_img_path = os.path.join('/home/penk1ng/Dataset/Visdrone_track/gendataset',video_path[0],video_path[1],video_path[2],pre_image_id+'.jpg')
        pre_label_path = os.path.join('/home/penk1ng/Dataset/Visdrone_track/gendataset', ann_path[0],ann_path[1],ann_path[2], pre_image_id+'.txt')
        if not os.path.isfile(pre_img_path):
            pre_label_path = label_path
            pre_img_path = img_path
            pre_imgs = imgs
