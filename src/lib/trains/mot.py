...
from lib.models.losses import GBF, HmLoss
...
class McMotLoss(torch.nn.Module):
    def __init__(self, opt):
        ...
        # self.GBF_pre = GBF()
        # num_class = 5
        self.HmLoss = HmLoss(num_classes=opt.num_classes)
        self.HmLoss_pre = HmLoss(num_classes=opt.num_classes)
        self.crit = torch.nn.MSELoss() if opt.mse_loss else FocalLoss()
        self.crit_reg = RegL1Loss() if opt.reg_loss == 'l1' else \
            RegLoss() if opt.reg_loss == 'sl1' else None  # L1 loss or smooth l1 loss
        self.crit_wh = torch.nn.L1Loss(reduction='sum') if opt.dense_wh else \
        ...
        self.dummy_target = torch.zeros(1, dtype=torch.long).to(opt.device)
    
    def reset_hm_grad(self):
        self.HmLoss.reset_grads()
        self.HmLoss_pre.reset_grads()
    ...
    def forward(self, outputs, batch):
        ...
        device = torch.device("cuda:0")  # replace "cuda:0" with the relevant device id
        hm_loss = torch.tensor(0.0, device=opt.device)
        wh_loss = torch.tensor(0.0, device=opt.device)
        off_loss = torch.tensor(0.0, device=opt.device)
        reid_loss = torch.tensor(0.0, device=opt.device)

        pre_hm_loss = torch.tensor(0.0, device=opt.device)
        pre_wh_loss = torch.tensor(0.0, device=opt.device)
        pre_off_loss = torch.tensor(0.0, device=opt.device)
        pre_reid_loss = torch.tensor(0.0, device=opt.device)
        for s in range(opt.num_stacks):
            # ----- Detection loss
            ...
            hm_loss += self.HmLoss(output['hm'], batch['hm'],batch['wh'],batch['location_class']) / opt.num_stacks
            pre_hm_loss += self.HmLoss_pre(pre_output['hm'], batch['pre_hm'],batch['pre_wh'],batch['pre_location_class']) / opt.num_stacks
            ...
            if opt.id_weight > 0:  # if ReID is needed
                ...
                for cls_id, id_num in self.nID_dict.items():
                    ...
                    # if inds[0].shape[0] == 0 or pre_inds[0].shape[0] == 0:
                    #     continue
                    if inds[0].shape[0] == 0 and inds[0].shape[0] == 0 and pre_inds[0].shape[0] == 0:
                        dummy_input = torch.randn(1, self.emb_dim).to(opt.device)
                        dummy_cls_id_pred = self.classifiers[str(cls_id)].forward(dummy_input).contiguous()
                        dummy_loss = self.focal_loss_dict[str(cls_id)](dummy_cls_id_pred, self.dummy_target)
                        unused_feature_loss = pre_output['id'].pow(2).sum()
                        pre_reid_loss += 1e-7 * unused_feature_loss
                        pre_reid_loss += 1e-3 * dummy_loss
                        
                        dummy_input = torch.randn(1, self.emb_dim).to(opt.device)
                        dummy_cls_id_pred = self.classifiers[str(cls_id)].forward(dummy_input).contiguous()
                        dummy_loss = self.focal_loss_dict[str(cls_id)](dummy_cls_id_pred, self.dummy_target)
                        unused_feature_loss = output['id'].pow(2).sum()
                        reid_loss += 1e-7 * unused_feature_loss
                        reid_loss += 1e-3 * dummy_loss
                        # print(f'unused_feature_loss = {1e-7 * unused_feature_loss}, dummy_loss = {1e-3 * dummy_loss}')
                    elif pre_inds[0].shape[0] == 0:
                        dummy_input = torch.randn(1, self.emb_dim).to(opt.device)
                        dummy_cls_id_pred = self.classifiers[str(cls_id)].forward(dummy_input).contiguous()
                        dummy_loss = self.focal_loss_dict[str(cls_id)](dummy_cls_id_pred, self.dummy_target)
                        unused_feature_loss = pre_output['id'].pow(2).sum()
                        ...
                        pre_reid_loss += self.focal_loss_dict[str(cls_id)](pre_cls_id_pred, pre_cls_id_target)
        ...
        if opt.id_weight > 0:
            now_loss = torch.exp(-self.s_det) * det_loss \
        ...
        if opt.id_weight > 0:
            loss_stats = {'loss': now_loss,
                          ...
                          'pre_id_loss': pre_reid_loss,
                          }
            # for loss_name in loss_stats:
            #     print(f'type = {type(loss_stats[loss_name])},loss_stats[{loss_name}] = {loss_stats[loss_name]}')
        else:
            loss_stats = {'loss': loss,
                          'hm_loss': hm_loss,
                          'wh_loss': wh_loss,
...
class MotTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        ...
        # loss = MotLoss(opt)
        # loss = MyMcMotLoss(opt)  # multi-class multi-object tracking loss
        loss = McMotLoss(opt) 
