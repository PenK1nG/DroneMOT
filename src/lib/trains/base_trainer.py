...
from torch.cuda.amp.grad_scaler import GradScaler as GradScaler
...
class ModleWithLoss(torch.nn.Module):
    ...
    def forward(self, batch):
        outputs = self.model.forward(batch['pre_image'],batch['input'],batch['pre_hm'],batch['hm'])
        loss, loss_stats = self.loss.forward(outputs=outputs, batch=batch)

        return outputs[-1], loss, loss_stats
    
    def reset_hm_grad(self):
        self.loss.reset_hm_grad()
