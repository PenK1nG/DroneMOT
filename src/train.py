...
def run(opt):
    ...
    os.environ['NCCL_SOCKET_IFNAME'] = 'enp5s0'
    # torch.distributed.init_process_group(backend='nccl')
    # torch.distributed.init_process_group(backend='nccl', init_method='tcp://10.7.48.82:23456',
    #                         rank=0, world_size=1)
    ...
    if opt.is_debug:
        if opt.multi_scale:
            ...
            train_loader = torch.utils.data.DataLoader(dataset=dataset,
                                                       ...
                                                       shuffle=True,
                                                       pin_memory=True,
                                                       drop_last=False)  # debug时不设置线程数(即默认为0)
    else:
        if opt.multi_scale:
            train_loader = torch.utils.data.DataLoader(dataset=dataset,
                                                       batch_size=opt.batch_size,
